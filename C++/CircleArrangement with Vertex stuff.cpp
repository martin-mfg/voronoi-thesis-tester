...
//		cout << "Points found in all faces: " << result.size() << endl;
		...
		Circle_Arrangement_Type demo_arrangement = Circle_Arrangement_Type( arrangement );
		Circle_Arrangement_Type::Vertex_handle vertex;
		CircleTrait_Point origin( 100, 100 );
		vertex = insert_point( demo_arrangement, origin ); // http://www.cgal.org/Manual/3.3/doc_html/cgal_manual/Arrangement_2_ref/Function_insert_point.html
		
		Circle_Arrangement_Type::Face_iterator face = demo_arrangement.faces_begin();
		if( face->isolated_vertices_begin() != face->isolated_vertices_end() ) {
		cout << "first face contains (100,100)\n";
		
		}
		face++;
		if( face->isolated_vertices_begin() != face->isolated_vertices_end() ) {
		cout << "second face contains (100,100)\n";
		
		}
		  
		bool success = remove_vertex( demo_arrangement, vertex ); // http://www.cgal.org/Manual/3.3/doc_html/cgal_manual/Arrangement_2_ref/Function_remove_vertex.html
		if(success)
		    cout<<"vertex was successfully removed\n";
