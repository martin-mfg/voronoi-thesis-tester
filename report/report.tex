...
The numbers used by our algorithm are mostly floating point numbers. While calculations involving floating point numbers are usually not totally precise, CGAL offers the possibility to perform 100% exact calculations. The disadvantage of using exact calculations everywhere is that this leads to a noticeable performance decrease. Therefore, we decided to use inexact calculations for the construction of the circles that are to be covered by our algorithm. Exact calculations are only used for finding a point in every face.
Thus, the circles used in the algorithm are usually a little bit smaller or bigger than they should be, or aren't located exactly where they should be. It is therefore theoretically possible that a set of blue points is found, that covers all circles when it shouldn't, or vice versa. But this is not a problem, because for the Delaunay triangulation, that is later constructed using the found points and that is used to decide whether a solution to the considered problem is found, does not depend on the calculated circles and their precision. So, even if something goes wrong during circle construction, it does not affect the correctness of the returned solution.

The solution retured by our program doesn't necessarily have to be correct though, because the construction of the Delaunay triangulation does also use inexact calculations. It's therefore possible that a solution returned by our program is not a real solution, or that a found solution is considered insufficient by mistake. This may, but doesn't have to mean that later, a solution using more blue points than necessary will be accepted and returned. The only way to guarantee that the returned solutions are always correct and minimal would be to do all calculations in an exact way. As this would lead to unacceptable performance and the probability of mistakes is very small, we decided not to use exact calculations everywhere.
...
This latter approach is computationally easy, but does not fully fulfill the requirement of the algorithm. As explained above, it can not lead to incorrect solutions being returned by our program, but it might happen that the returned solution is not minimal. Again, this unlikely and the performance gain of using the inexact method is large, so that we decided to use this approach instead of the one explained in section \hyperref[sec:findPoints]{4.3}. But we also implemented the exact approach, so that switching over is easy. \newline 
The gain in time is shown in the graph below.
